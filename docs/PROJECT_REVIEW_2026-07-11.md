...
1. **修复 launchd / `gen_launchd.py` 中硬编码的旧用户路径**（当前指向 `/Users/xiaosheji/...`），否则本机安装会静默跑错或跑不起来。
...
- `scripts/gen_launchd.py`：`PYTHON = "/Users/xiaosheji/projects/market-monitor/.venv/bin/python"`
...
| # | 位置 | 问题 | 建议 |
|---|------|------|------|
| B1 | `scripts/gen_launchd.py` + `launchd/*.plist` | Python / WorkingDirectory 硬编码为 `/Users/xiaosheji/projects/market-monitor` | 用 `PROJECT_ROOT` + `sys.executable` 或 env 生成；重新 `gen_launchd` 并安装 |
